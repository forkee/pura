...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "pura (or specifically: privatepay, instapay, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Pura specific functionality (Masternodes, PrivatePay, InstaPay, Governance) (0-1, default: %u)"), 0));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Pura Core Developers SK"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Pura Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Pura Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Pura Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single Pura Core process is using the data directory.
    ...
    try {
        ...
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pura Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // ********************************************************* Step 11c: update block tip in Pura modules
    ...
    // ********************************************************* Step 11d: start pura-ps-<smth> threads
