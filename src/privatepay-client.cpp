...
bool CPrivatePayClient::MakeCollateralAmounts(const CompactTallyItem& tallyItem, bool fTryDenominated, CConnman& connman)
{
    ...
    if(!fSuccess) {
        LogPrintf("CPrivatePayClient::MakeCollateralAmounts -- ONLY_NONDENOMINATED_NOT1000IFMN Error: %s\n", strFail);
        ...
        if(fTryDenominated) {
            ...
            if(!pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                ...
                LogPrintf("CPrivatePayClient::MakeCollateralAmounts -- ONLY_NOT1000IFMN Error: %s\n", strFail);
