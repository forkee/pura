...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Pura address for receiving payments to this account.\n"
            "\nArguments:\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress \"puraaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount use_is use_ps )\n"
            ...
            "                             The recipient will receive less amount of Pura than you enter in the amount field.\n"
...
UniValue instapaytoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "instapaytoaddress \"puraaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            ...
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("instapaytoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" 0.1")
            + HelpExampleCli("instapaytoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleCli("instapaytoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" 0.1 \"\" \"\" true")
            + HelpExampleRpc("instapaytoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\", 0.1, \"donation\", \"seans outpost\"")
        );
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a pura address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The pura address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less puras than you enter in their corresponding amount field.\n"
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Pura address: ")+name_);
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Wallet encrypted; Pura Core server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
